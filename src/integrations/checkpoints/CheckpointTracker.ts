...
class CheckpointTracker {
	...
	public static async create(taskId: string, provider?: ClineProvider): Promise<CheckpointTracker | undefined> {
		try {
			...
			const enableCheckpoints = vscode.workspace.getConfiguration("ecline").get<boolean>("enableCheckpoints") ?? true
	...
	private static async getWorkingDirectory(): Promise<string> {
		...
		if (!cwd) {
			throw new Error("No workspace detected. Please open Ecline in a workspace to use checkpoints.")
		}
	...
	public async initShadowGit(): Promise<string> {
		...
		if (await fileExistsAtPath(gitPath)) {
			...
			await git.addConfig("user.name", "Ecline Checkpoint")
